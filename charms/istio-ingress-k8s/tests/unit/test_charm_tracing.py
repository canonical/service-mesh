...
def test_charm_tracing_requests_otlp_http(istio_ingress_context):
    ...
    state_out = istio_ingress_context.run(
        istio_ingress_context.on.relation_joined(relation),
...
def test_charm_tracing_sets_destination_from_relation(istio_ingress_context):
    ...
    with patch("ops.tracing.set_destination") as mock_set_destination:
        istio_ingress_context.run(
            istio_ingress_context.on.relation_changed(relation),
            State(leader=True, relations=[relation]),
        )
    mock_set_destination.assert_called_with(url=f"{TRACING_ENDPOINT}/v1/traces", ca=None)


def test_charm_tracing_clears_destination_without_relation(istio_ingress_context):
    """With no charm-tracing relation, the tracing destination is cleared."""
    with patch("ops.tracing.set_destination") as mock_set_destination:
        istio_ingress_context.run(
            istio_ingress_context.on.start(),
            State(
                leader=True,
                relations=[],
                containers=[Container("metrics-proxy", can_connect=True)],
            ),
        )
    mock_set_destination.assert_called_with(url=None, ca=None)


def test_charm_tracing_emits_spans(istio_ingress_context):
    ...
    spans = istio_ingress_context.trace_data
    ...
    assert resource_attributes["charm"] == "istio-ingress-k8s"
    assert resource_attributes["charm_type"] == "IstioIngressCharm"
