...
"""Grafana dashboard relation steps for the envoy-controller-k8s suite."""
...
CONTROLLER_DASHBOARD = "Envoy Gateway - Controller Health"
DATA_PLANE_DASHBOARD = "Envoy Gateway - Data Plane Metrics"
...
@then("the dashboard JSON includes a controller health dashboard")
def controller_dashboard_present(juju: Juju) -> None:
    """Assert the controller health dashboard is published."""
    assert CONTROLLER_DASHBOARD in _dashboard_titles(juju)


@then("the dashboard JSON includes a data plane metrics dashboard")
def data_plane_dashboard_present(juju: Juju) -> None:
    """Assert the data plane metrics dashboard is published."""
    assert DATA_PLANE_DASHBOARD in _dashboard_titles(juju)
