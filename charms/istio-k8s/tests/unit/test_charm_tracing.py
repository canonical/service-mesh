...
def test_charm_tracing_requests_otlp_http(istio_core_context):
    ...
    state_out = istio_core_context.run(
        istio_core_context.on.relation_joined(relation),
...
def test_charm_tracing_sets_destination_from_relation(istio_core_context):
...
def test_charm_tracing_emits_spans(istio_core_context):
    ...
    assert resource_attributes["charm"] == "istio-k8s"
    assert resource_attributes["charm_type"] == "IstioCoreCharm"
